...
public class DefectoJpaController implements Serializable {
    ...
    public boolean RegistroDefecto(int id_familia, String consecutivo, String defecto, int id_clasificacion, String descripcion, String usuario_registro) {
        EntityManager em = getEntityManager();
        em.getTransaction().begin();
        try {
            Query q = em.createNativeQuery("CALL `sp_dft_r_defecto`('" + id_familia + "','" + consecutivo + "','" + defecto + "','" + id_clasificacion + "','" + descripcion + "','" + usuario_registro + "')");
