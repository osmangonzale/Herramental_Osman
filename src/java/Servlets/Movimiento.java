...
public class Movimiento extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            String nom_rol = sesion.getAttribute("Nombre_rol").toString();
...
//</editor-fold>
            ...
            String filtro = "", descripcion_herramental = "", usa_firmas, fecha, descripcion, ids_herramental, codigo, producto, lote, cavidades, herramental, id_usuarios, descripcion_cavidadesT, programacion = "";
...
//</editor-fold>
            switch (opc) {
                ...
                case 1:
                    ...
                    break;
//</editor-fold>
                //<editor-fold defaultstate="collapsed" desc="CASO 2 REGISTRAR MOVIMIENTO">
                case 2:
                    try {
                        id_maquina = Integer.parseInt(request.getParameter("idM"));
                    } catch (Exception ex) {
                    ...
                    try {
                        id_tipoM = Integer.parseInt(request.getParameter("movimiento"));
                    } catch (Exception ex) {
                        id_tipoM = 1;
                    }
                    ...
                    if (!obj_herramental[3].toString().contains("[" + id_maquina + "]")) {
                        jpa_herramental.ModificarHerramental(id_Herramental, obj_herramental[4].toString(), descripcion_herramental, (Integer) obj_herramental[1], obj_herramental[3].toString() + "[" + id_maquina + "]", nombre_Usuario);
                    }
...
//                        resultado = jpa_movimiento.RegistroMovimiento(id_maquina, id_Herramental, id_tipoM, fecha, producto, lote, cavidad, cavidadDes, cavidades, descripcion, "[" + nombre_Usuario + "/" + area + "]", descripcion_cavidadesT, usa_firmas);
                    ...
                    if (id_maquina == 1) {
                        request.getRequestDispatcher("Herramental?opc=5&idH=" + id_Herramental + "&idP=" + 0 + "&idMV=" + 0 + "&idS=" + 0 + "").forward(request, response);
                    } else {
                        if (id_tipoM == 2 || id_tipoM == 3) {
...
//                            jpa_maquina.ModificarEstadoMaquina(id_maquina, 3);
                            ...
                            request.getRequestDispatcher("Movimiento?opc=1&idM=" + id_maquina + "&idMV=" + 0 + "&idP=" + 0 + "&idS=" + 0 + "").forward(request, response);
                        }
                    }
                    break;
//</editor-fold>
